  ...
  const handleApplyCoupon = () => {
    ...
    if (code === "WELCOME50") {
      ...
      if (subtotal >= 499) {
        ...
        setCouponError("SAVE100 requires a minimum order of ₹499")
      }
    } else if (code === "FREEDEL") {
      if (subtotal >= 299) {
        setAppliedCoupon({ code: "FREEDEL", discount: deliveryFee })
      } else {
        setCouponError("FREEDEL requires a minimum order of ₹299")
      }
    } else {
        ...
        <div className="grid gap-6 lg:grid-cols-3">
            ...
            >
              ...
              {appliedCoupon && (
                <div className="mt-2 flex items-center gap-2 rounded-lg bg-green-500/10 p-2 text-sm text-green-500">
                  ...
                  <span>Coupon {appliedCoupon.code} applied! You save ₹{appliedCoupon.discount.toFixed(0)}</span>
                </div>
              )}
              {couponError && (
          ...
          <div className="lg:sticky lg:top-20 lg:self-start">
            ...
            >
              ...
              {restaurant && (restaurant.min_order ?? 0) > subtotal && (
                <p className="mt-4 rounded-lg bg-destructive/10 p-3 text-xs text-destructive">
                  Minimum order is Rs.{(restaurant.min_order ?? 0).toFixed(0)}. Add Rs.
                  {((restaurant.min_order ?? 0) - subtotal).toFixed(0)} more to checkout.
                </p>
              )}
              ...
              <Button
                ...
                disabled={
                  ...
                  (restaurant ? subtotal < (restaurant.min_order ?? 0) : false)
                }
